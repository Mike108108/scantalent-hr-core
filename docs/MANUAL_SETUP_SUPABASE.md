...
4. Открой **SQL Editor**.
5. Скопируй **актуальную** версию файла `supabase/migrations/202606050001_initial_schema.sql` из репозитория.
6. Вставь в SQL Editor и нажми **Run**.
   - Если первый запуск ранее упал с ошибкой — просто запусти migration **ещё раз целиком**. Скрипт идемпотентный: `IF NOT EXISTS`, `DROP POLICY IF EXISTS`, `DROP TRIGGER IF EXISTS`.
7. Затем скопируй содержимое `supabase/seed/202606050001_reference_seed.sql`.
8. Вставь в SQL Editor и снова нажми **Run** (только после успешного migration).
...
**Migration** — это SQL-скрипт, который создаёт структуру базы: таблицы, связи, индексы, политики доступа. Его нужно выполнить при создании нового Supabase project. Порядок внутри скрипта: сначала таблицы, потом функции/триггеры/политики. При ошибке можно перезапустить тот же файл целиком.

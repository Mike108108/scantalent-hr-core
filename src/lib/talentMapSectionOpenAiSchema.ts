...
export function buildTalentMapSectionSystemPrompt(params: {
  ...
  const depthBlock = [
    ...
    '',
    'Целевые объёмы для этого профиля:',
    `- Base blocks: ${depthProfile.base_points_per_block.min}–${depthProfile.base_points_per_block.max} пункта в каждом блоке`,
    `- source_chips: ${depthProfile.source_chips_target.min}–${depthProfile.source_chips_target.max} (только из входа)`,
    `- pro.source_logic: ${depthProfile.source_logic_target.min}–${depthProfile.source_logic_target.max} источников`,
    `- interpretation_limits: ${depthProfile.interpretation_limits_target.min}–${depthProfile.interpretation_limits_target.max}`,
    `- reality_checks: ${depthProfile.reality_checks_target.min}–${depthProfile.reality_checks_target.max}`,
    `- summary key_conditions: ${depthProfile.summary_key_conditions_target.min}–${depthProfile.summary_key_conditions_target.max}`,
    `- summary potential_risks: ${depthProfile.summary_risks_target.min}–${depthProfile.summary_risks_target.max}`,
    '',
    'Принцип depth:',
